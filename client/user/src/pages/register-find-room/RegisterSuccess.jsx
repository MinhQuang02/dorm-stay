...
export default function RegisterSuccess() {
  ...
  useEffect(() => {
    const data = sessionStorage.getItem('bookingSummary');
    if (data) setSummary(JSON.parse(data));

    // Xóa session sau khi đã hiển thị thành công để tránh đăng ký lặp
    return () => {
      sessionStorage.removeItem('selectedRoom');
      sessionStorage.removeItem('currentBookingPhieu');
  ...
  return (
    <MainLayout title="Room Rental Registration" mainClassName="flex-1 flex flex-col items-center py-16 px-6">
      <div className="flex flex-col items-center mb-12 text-center">
        ...
        <h2 className="text-[32px] font-bold text-gray-900 mb-3">Confirmed!</h2>
        <p className="text-gray-500">Hi {summary.customer.hoTen}, your booking for {summary.roomName} is confirmed.</p>
      </div>

      <div className="w-full max-w-3xl border-t border-gray-100 pt-8">
          <div className="flex justify-between items-center mb-4">
            <span className="text-gray-900 font-bold">Viewing Date</span>
            <span className="text-gray-500">{summary.date}</span>
          </div>
          <p className="text-xs text-center text-gray-400 mt-10">Our staff will contact you via {summary.customer.sdt} soon.</p>
      </div>
    </MainLayout>
  );
}